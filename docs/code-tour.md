...
> **解决什么**：用两条链路（写路径、读路径）把 38 个类串起来，告诉你「从哪个入口进来、每一步落在哪个类、哪个方法」，并标出接口化的「缝」。
...
- 校验 question 非空 → 调 `RagService.ask(question, version, conversationId)` 拿 `Answer(Flux<String> stream, List<Source> sources)`。
- 把 `Flux` 订成 SSE：逐 token 发，末尾发一个 `sources` 事件（引用 URL 列表，含版本）。

### 2.2 `RagService.ask()` —— 读路径唯一的编排点

`service/RagService.java`，七步（`ask` 方法里按顺序）：
...
| ⑤ | 拼编号上下文 | `buildContext(docs)` → `[n] title (v版本) — url + 正文` |
| ⑥ | 注入多轮历史 | `conversationId` 非空时，从 `ConversationStore`（SQLite）读最近消息拼进 system；流式结束后把 user/assistant 两条消息写回 SQLite |
...
### 2.3 会话持久化：`conversation/` 包（SQLite）

`conversation/ConversationStore.java`：用 SQLite（`data/conversations.db`）存三张表——`conversation` / `message` / `folder`。`RagService` 生成前把最近的 10 条消息拼进 prompt 作为多轮上下文、流式结束后把 user/assistant 两条消息写回；`ConversationController`（`/api/*`）提供会话/文件夹的 CRUD，前端聊天 UI 直接读它。SQLite 是嵌入式单文件、零运维，会话状态不依赖 pgvector RDS，且重启不丢。

### 2.4 检索：`HybridDocumentRetriever`（`retrieve/HybridDocumentRetriever.java`）
...
### 2.5 重排：`DashScopeRerankPostProcessor`（`rerank/`）
