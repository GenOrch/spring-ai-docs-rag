...
```
POST /ask {question, version?, conversationId?}
  ...
  → RagService：拼编号上下文 [n] 标题 (v版本) — URL + 正文；带 conversationId 时注入历史轮次
  → ChatClient 流式生成
  → SSE 逐 token + [n] 引用 + sources（URL 列表，含版本）
```
...
├─ observability/  README + prometheus.yml + grafana 数据源/大盘
...
│  ├─ conversation/  ConversationStore + ConversationController + 模型（SQLite 会话/消息/文件夹）
...
- `org.xerial:sqlite-jdbc` 3.46.1.0（SQLite 会话/消息/文件夹持久化）
...
- `GET /`（demo 页，聊天 UI：会话列表 + 文件夹 + 多轮 + 引用）
- `POST /ask`（SSE 问答，body 可带 `version` 过滤、`conversationId` 多轮）
- `POST /admin/ingest`（灌库）
- `GET /admin/eval`（评估）
- `/mcp`（MCP server，streamable HTTP，受 `AdminAuthFilter` 保护）
- `/api/conversations` + `/api/folders`（会话/文件夹 CRUD，SQLite 持久化）
