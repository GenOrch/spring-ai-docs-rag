...
| `RAG_MCP_AUDIT_FILE_ENABLED` | `false` | 是否把请求审计追加落盘（JSONL，按大小轮转 ~10MB、保留 3 份） |
...
### 2.1 非环境变量的可调参数（在 `application.yml` 的 `app.rag.*`，改后重启生效）

| 参数 | 默认 | 作用 |
|---|---|---|
| `app.rag.chunk.default-chunk-size` | `500` | 切 chunk 的 token 目标长度 |
| `app.rag.chunk.min-chunk-size-chars` / `min-chunk-length-to-embed` / `max-num-chunks` | `100` / `10` / `10000` | 切分保护参数 |
| `app.rag.retrieve.vector-top-k` / `keyword-top-k` / `fusion-top-n` / `rrf-k` | `20` / `20` / `20` / `60` | 两腿召回数、融合条数与 RRF 参数 |
| `app.rag.rerank.top-n` | `5` | 重排后进入上下文的条数 |
| `app.rag.system-prompt` | （见 `application.yml`） | 生成用系统提示词 |
...
- 语料（clone 下来的 `.adoc`）、会话数据（SQLite `data/conversations.db`）与可选审计日志都在 `data/`，同样被 `.gitignore` 忽略（向量在 pgvector 表，不在本地文件）。
- 语料快照（`.adoc` 源码）已作为独立 Release 提供（`corpus-v1.0.0`，249 页 + README + Apache-2.0 LICENSE），由使用者自备 API key 重新向量化。
