...
class RagServiceTest {
    ...
    @Test
    void buildContextAnnotatesTitleAndVersion() {
        Document doc = Document.builder()
                .id("c1")
                .text("body text")
                .metadata(Map.of(
                        DocumentMeta.TITLE, "Chat Client",
                        DocumentMeta.SOURCE_URL, "https://x/chat.html",
                        DocumentMeta.VERSION, "2.0.1"))
                .build();

        String context = RagService.buildContext(List.of(doc));

        assertThat(context)
                .contains("[1] Chat Client (v2.0.1) — https://x/chat.html")
                .contains("body text");
    }

    @Test
    void buildContextOmitsVersionWhenAbsent() {
        Document doc = Document.builder()
                .id("c1")
                .text("body text")
                .metadata(Map.of(DocumentMeta.TITLE, "Chat Client", DocumentMeta.SOURCE_URL, "https://x/chat.html"))
                .build();

        String context = RagService.buildContext(List.of(doc));

        assertThat(context)
                .contains("[1] Chat Client — https://x/chat.html")
                .doesNotContain("(v");
    }
}
