...
- **Cited answers**: the LLM cites `[n]`, and `/ask` streams a final `sources` SSE event
  with the URL list — each source is tagged with its Spring AI version.
- **Multi-turn + conversation persistence**: `/ask` accepts an optional `conversationId` to
  recall earlier turns; conversations, messages and folders are stored in a local SQLite
  database (`data/conversations.db`), so they survive restarts.
...
- **Demo page**: a ChatGPT-style chat UI at `/` — conversation list + folders, multi-turn Q&A, markdown, citations and per-source version tags.
...
3. Evaluation — retrieval / rerank / source hit rates:
   ...
   ```
4. Open <http://localhost:8080> for the chat UI — a conversation list with folders,
   multi-turn follow-ups, markdown rendering, clickable `[n]` citations, and per-source version tags:
